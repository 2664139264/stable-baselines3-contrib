...
class GruRolloutBuffer(RolloutBuffer):
    """
    Rollout buffer that also stores the GRU cell and hidden states.

    :param buffer_size: Max number of element in the buffer
    :param observation_space: Observation space
    :param action_space: Action space
    :param hidden_state_shape: Shape of the buffer that will collect gru states
        (n_steps, gru.num_layers, n_envs, gru.hidden_size)
    :param device: PyTorch device
    :param gae_lambda: Factor for trade-off of bias vs variance for Generalized Advantage Estimator
        Equivalent to classic advantage when set to 1.
    :param gamma: Discount factor
    ...
    def add(self, *args, gru_states: RNNStates, **kwargs) -> None:
        """
        :param hidden_states: LSTM cell and hidden state
        """
        self.hidden_states_pi[self.pos] = np.array(gru_states.pi.cpu().numpy())
        self.hidden_states_vf[self.pos] = np.array(gru_states.vf.cpu().numpy())
    ...
    def get(self, batch_size: Optional[int] = None) -> Generator[RecurrentRolloutBufferSamples, None, None]:
        ...
        # Prepare the data
        if not self.generator_ready:
            # hidden_state_shape = (self.n_steps, gru.num_layers, self.n_envs, gru.hidden_size)
            # swap first to (self.n_steps, self.n_envs, gru.num_layers, gru.hidden_size)
    ...
    def _get_samples(
        ...
        gru_states_pi = self.hidden_states_pi[batch_inds][self.seq_start_indices].swapaxes(0, 1)
            # 1. (n_envs * n_steps, n_layers, dim) -> (batch_size, n_layers, dim)
            # 2. (batch_size, n_layers, dim)  -> (n_seq, n_layers, dim)
            # 3. (n_seq, n_layers, dim) -> (n_layers, n_seq, dim)

        gru_states_vf = self.hidden_states_vf[batch_inds][self.seq_start_indices].swapaxes(0, 1)
            # (n_envs * n_steps, n_layers, dim) -> (n_layers, n_seq, dim)
        gru_states_pi = self.to_torch(gru_states_pi).contiguous()
        gru_states_vf = self.to_torch(gru_states_vf).contiguous()
        ...
        return GruRolloutBufferSamples(
            ...
            advantages=self.pad_and_flatten(self.advantages[batch_inds]),
            returns=self.pad_and_flatten(self.returns[batch_inds]),
            gru_states=RNNStates(gru_states_pi, gru_states_vf),
            episode_starts=self.pad_and_flatten(self.episode_starts[batch_inds]),
            mask=self.pad_and_flatten(np.ones_like(self.returns[batch_inds])),
        )
