...
from sb3_contrib.ppo_recurrent.policies import (
    CnnLstmPolicy, MlpLstmPolicy, MultiInputLstmPolicy,
    #########
    MlpGruPolicy,
)
...
class GruPPO(OnPolicyAlgorithm):
    """
    Proximal Policy Optimization algorithm (PPO) (clip version)
    with support for recurrent policies (Gru).
    ...
    def _setup_model(self) -> None:
        ...
        # hidden state for actor and critic
        self._last_gru_states = RNNStates(
            pi=th.zeros(single_hidden_state_shape, device=self.device),
            vf=th.zeros(single_hidden_state_shape, device=self.device),    
        )
    ...
    def collect_rollouts(
        ...
        """
        assert isinstance(
            rollout_buffer, GruRolloutBuffer
        ), f"{rollout_buffer} doesn't support recurrent policy"
        ...
        while n_steps < n_rollout_steps:
            ...
            for idx, done_ in enumerate(dones):
                if (
                    ...
                    terminal_obs = self.policy.obs_to_tensor(infos[idx]["terminal_observation"])[0]
                    with th.no_grad():
                        terminal_gru_state = gru_states.vf[:, idx : idx + 1, :].contiguous()
                        # terminal_gru_state = None
                        episode_starts = th.tensor([False], dtype=th.float32, device=self.device)
        ...
        with th.no_grad():
            # Compute value for the last timestep
            episode_starts = th.tensor(dones, dtype=th.float32, device=self.device)
            values = self.policy.predict_values(obs_as_tensor(new_obs, self.device), gru_states.vf, episode_starts)
